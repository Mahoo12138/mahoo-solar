  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <aside className="side-panel telemetry-panel">
        ...
        <div className="power-hero"><div className="metric-label"><Zap size={14} />{snapshot.weatherSource === 'forecast' ? '天气修正功率' : snapshot.weatherSource === 'scenario' ? '情景模拟功率' : '晴空回退估算'}</div><div className="power-value" data-testid="solar-power">{snapshot.solarInputPower.toFixed(1)}<span>W</span></div><MetricBar value={snapshot.solarInputPower / panelConfig.ratedPower} accent="cyan" /><div className="metric-range"><span>晴空参考 {snapshot.clearSkyPower.toFixed(1)} W</span><span>额定 {panelConfig.ratedPower} W</span></div></div>
